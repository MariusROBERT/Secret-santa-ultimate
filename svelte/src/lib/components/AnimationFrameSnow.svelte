<!-- From https://svelte.dev/playground/3fe590584ad348cebd053e3f1c4f1b0e?version=5.43.3 -->
<script>
    ...
    function loop(timestamp) {
      ...
      snowflakes = snowflakes.map(flake => {
        if (flake.y >= 100) {
          ...
          flake.rotation += framesCompleted;
        }
...
<div class="snowframe" aria-hidden="true">
  {#each snowflakes as flake}
    <div
      class="snowflake {flake.scale > 0.8 ? 'z-10' : 'z-0'}"
